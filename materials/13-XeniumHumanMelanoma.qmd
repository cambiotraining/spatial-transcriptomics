...
Perform SCTransform on the Xenium data to normalise and scale the data.
Then, run PCA for dimensionality reduction to 30 principal components using all measured features.
Finally create a UMAP reduction for future visualisation.
...
Before we run clustering, we need to find the nearest neighbours of each cell in the dataset.
...
We can visualise the clusters using the UMAP reduction we created earlier.
The `DimPlot` function allows us to plot the UMAP with the clusters colored by their cluster identity.
For spatial data, we can also visualise the clusters on the spatial coordinates of the cells, by using the `ImageDimPlot` function.
...
We can then visualise the top marker genes for each cluster using the `FeaturePlot` function.
...
We will use the `lambda` parameter to control the strength of the spatial clustering, and the `k_geom` parameter to control the number of nearest neighbours used in the clustering.
...
We then need to create a new PCA reduction for the Banksy results, which will be used for visualisation and further analysis.
...
After running the Banksy analysis, we can visualise the spatial clusters identified by Banksy on the spatial image.
...
We can then visualise the top marker genes for each Banksy cluster using the `FeaturePlot` function.
...
We can also visualise the overlap between the Banksy clusters and the Seurat clusters using a heatmap.
...
This analysis provides a comprehensive overview of how to analyse Xenium data and how to interpret the results.
...
This is helpful for specific research questions, like studying expression of a set of genes in a specific tissue context, but often does not allow for a comprehensive analysis of all cells present in the tissue.
