...
Now we can compare the two results. The first result with a lambda value of 0.4 shows more transcriptional variation, while the second result with a lambda value of 0.8 shows more spatially coherent clusters. Depending on the biological question, one may be more relevant than the other.

The second parameter we should consider changing is `k_geom`, which controls the number of nearest neighbors used to construct the spatial graph. A higher value of `k_geom` will result in a more connected graph, while a lower value will result in a more fragmented graph. We will run BANKSY again with a lower `k_geom` value of 15 and compare the results.
