...
In the context of spatial transcriptomics, deconvolution can help identify the proportions of different cell types within spatially resolved samples.

- TODO: expand this discussion a bit more
- Discuss different tools and RCTD in particular

## Running RCTD

The RCTD (Robust Cell Type Decomposition) function is implemented in the Bioconductor package [`spacexr`](https://www.bioconductor.org/packages/release/bioc/html/spacexr.html).
...
The first step in running RCTD is to prepare the single-cell reference data and the spatial transcriptomics data in the appropriate formats.

For this example, we will use a preprocessed single-cell RNA-seq dataset as the reference.
We will have a quick look at the data to understand its structure.
...
DimPlot(ref, reduction = "umap", group.by = "subclass", label = TRUE) +
  NoLegend()
DimPlot(ref, reduction = "umap", group.by = "cluster", label = TRUE) +
  NoLegend()
```
...
We will use the subclass annotation for deconvolution, as it provides a good balance between granularity and interpretability.
We now need to update the Seurat object to the latest version (just in case it is an older version) and set the identities to the subclass annotation.
...
We will now prepare the reference object for RCTD.
This involves extracting the counts, cluster identities, and number of UMIs from the Seurat object and creating a Reference object.
If you use a different single-cell dataset, make sure that each celltype has a sufficient number of cells (at least 25) to ensure RCTD will work properly.
...
Next, we need to prepare the spatial transcriptomics data for RCTD.
We will convert the 'visium' Seurat object to a format compatible with RCTD.
...
Now we can create the RCTD object and run the deconvolution analysis.
We will use parallel processing to speed up the computation.
...
We can now add the results back into the Seurat visium object and visualize them.
...
We can see that most spots are classified as doublets.
We will in future analyses focus on the first cell type identified by RCTD, but you can also explore the second cell type or both together.
...
clustered_spatial1 <- SpatialDimPlot(
  visium,
  group.by = "Leiden_08",
  label = FALSE
)
first + clustered_spatial1
clustered_spatial2 <- SpatialDimPlot(
  visium,
  group.by = "Louvain_08",
  label = FALSE
)
...
This is actually not the case for the Visium v1 data we are using here, as each spot likely contains multiple cell types based on the spot size of 55um.
...
This will take a bit longer to compute as it estimates the proportions of all cell types in each spot and then adds these as metadata to the Seurat object.
We can now visualize the proportions of different cell types identified by RCTD.
...
max_celltype <- apply(RCTD@results$weights, 1, function(x) {
  ref_cell_types[which.max(x)]
})
visium <- AddMetaData(
  visium,
  metadata = max_celltype,
  col.name = "RCTD_max_celltype"
)
...
In this section, we have learned how to perform deconvolution of spatial transcriptomics data using the `RCTD` package.
We prepared both single-cell reference data and spatial transcriptomics data, ran the deconvolution analysis, and visualized the results.
Deconvolution can provide valuable insights into the cellular composition of spatially resolved samples, helping to better understand tissue architecture and function.
